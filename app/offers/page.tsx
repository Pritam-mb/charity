  ...
  return (
    <div>
      <div className="row-between" style={{ marginBottom: 18 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800 }}>Offerings Board</h1>
          <p className="muted" style={{ fontSize: 14, marginTop: 4 }}>
            Have something to provide? Offer food, books, time, or services as community offerings here.
          </p>
        </div>
      </div>
      ...
      <h2 className="section-title" style={{ marginTop: 24 }}>Available Offerings</h2>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          return (
            <div key={o.id} className="card" style={{ borderLeft: "4px solid var(--good)" }}>
              ...
              <div className="muted" style={{ fontSize: 13 }}>
                Offered by: <strong>{user?.display_name ?? "Unknown"}</strong> {user?.honor_badge ? `Honor x${user.honor_badge}` : ""}
                <br />
                Contact: {o.contact}
              </div>
            </div>
        ...
        {offers.length === 0 && (
          <div className="empty">No offerings posted yet. Be the first to share an offering!</div>
        )}
      </div>
    </div>
  );
